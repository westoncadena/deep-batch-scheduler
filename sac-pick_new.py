...
class SACBuffer:
    ...
    def finish_path(self, last_val=0):
        """
        Finish the trajectory, which may involve computing advantages, rewards-to-go, etc.
        """
...
def sac(workload_file, model_path, ac_kwargs=dict(), seed=0, 
    ...
    if pre_trained:
        ...
        r_ph, done_ph, next_obs_ph= placeholders(None, None, None)
        ...
        # SAC Objectives
        v_target = tf.minimum(q1, q2) - alpha * logp_pi
        q1_target = r_ph + gamma * (1 - done_ph) * (v_target - alpha * logp_pi)
        q2_target = r_ph + gamma * (1 - done_ph) * (v_target - alpha * logp_pi)
        q1_loss = 0.5 * tf.reduce_mean((q1 - q1_target) ** 2)
        q2_loss = 0.5 * tf.reduce_mean((q2 - q2_target) ** 2)
